...
export async function POST(req: Request) {
  ...
  try {
    ...
    //新規登録時に入力されたemailがemployeeにすでに存在する場合は登録できないようにする。
    ...
    await TenantModel.create(sanitizedBody)
    return NextResponse.json({ message: "新規登録しました。" }, { status: 200 })
  } catch (error: any) {
    if (error.code === 11000) { // connectDBの重複エラー
      return NextResponse.json({ message: `${error.keyValue.email} は既に登録されています。` }, { status: 400 })
    }
    // その他のエラー
