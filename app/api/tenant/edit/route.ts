...
export async function PUT(req: Request) {
  ...
  // console.log(body.lastCommentDate);
  ...
  if (!body.name || !body.email || !body.password) {
    if (body.lastCommentDate) {
      try {
        ...
        const res = await TenantModel.updateOne({ tenantId: body.tenantId }, { $set: { lastCommentDate: body.lastCommentDate } })
  ...
  try {
    ...
    //編集時に入力されたemailがemployeeテーブルに存在する場合は登録できないようにする。
    const employee = await EmployeeModel.findOne({ email: sanitizedBody.email })
    if (employee) {
      return NextResponse.json({ message: `このメールアドレスは既に登録されています。` }, { status: 400 })
    }

    await TenantModel.updateOne({ tenantId: body.tenantId }, sanitizedBody)// updateOne({インプットで渡ってきた値とDB内の値が一致したら}, {渡ってきた値をサニタイズしてアップデート})
